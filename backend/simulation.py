"""
Public-run agent generation service.

Creates user-submitted agents for the permanent public simulation.
Each pseudo-anon creator (UUID token stored client-side) gets one agent.
Agents are regular Agent rows — creator_token marks them as creator-owned.
...
def create_agent(
    ...
    *,
    ...
    """
    Generate and persist a new public-run agent into the permanent public run.
    ...
    """
    from simulation_run import get_or_create_public_run, AGENT_LIFETIME_DAYS
    ...
    if existing >= MAX_AGENTS_PER_TOKEN:
        raise ValueError("You already have an agent in the simulation.")
    ...
    # ── Persist ───────────────────────────────────────────────────────────────
    run_id = get_or_create_public_run(current_app._get_current_object())
    ...
    agent = Agent(
        ...
        expires_at=datetime.utcnow() + timedelta(days=AGENT_LIFETIME_DAYS),
    ...
    targets = _pick_follow_targets(agent_interests, existing_agents, FOLLOWS_PER_AGENT)
    ...
    def _do_intro():
        from engine import generate_intro_post
