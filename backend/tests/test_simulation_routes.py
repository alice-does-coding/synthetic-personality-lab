"""
Route-level tests for /api/simulation/* endpoints.
...
def test_create_agent_returns_201(client):
    with patch("simulation._generate_bio", return_value="A haunting presence."):
        resp = client.post("/api/simulation/agents", json=payload())
...
def test_create_agent_name_in_response(client):
    with patch("simulation._generate_bio", return_value="bio"):
        resp = client.post("/api/simulation/agents", json=payload(name="Cosmo Unit"))
    assert resp.get_json()["name"] == "Cosmo Unit"


def test_create_agent_bio_from_llm(client):
    with patch("simulation._generate_bio", return_value="Generated bio text."):
        resp = client.post("/api/simulation/agents", json=payload())
...
def test_create_agent_missing_token_returns_400(client):
    with patch("simulation._generate_bio", return_value="bio"):
        resp = client.post("/api/simulation/agents", json=payload(creator_token=""))
...
def test_create_agent_empty_name_returns_400(client):
    with patch("simulation._generate_bio", return_value="bio"):
        resp = client.post("/api/simulation/agents", json=payload(name=""))
    assert resp.status_code == 400


def test_create_agent_name_too_long_returns_400(client):
    with patch("simulation._generate_bio", return_value="bio"):
        resp = client.post("/api/simulation/agents", json=payload(name="x" * 51))
    assert resp.status_code == 400


def test_create_agent_description_too_short_returns_400(client):
    with patch("simulation._generate_bio", return_value="bio"):
        resp = client.post("/api/simulation/agents", json=payload(description="short"))
    assert resp.status_code == 400


def test_create_agent_description_too_long_returns_400(client):
    with patch("simulation._generate_bio", return_value="bio"):
        resp = client.post("/api/simulation/agents", json=payload(description="x" * 501))
    assert resp.status_code == 400
...
def test_duplicate_token_returns_400(client):
    t = str(uuid.uuid4())
    with patch("simulation._generate_bio", return_value="bio"):
        client.post("/api/simulation/agents", json=payload(creator_token=t))
        resp = client.post("/api/simulation/agents", json=payload(creator_token=t, name="Clone"))
    assert resp.status_code == 400
...
def test_list_agents_empty(client):
    resp = client.get("/api/simulation/agents")
...
def test_list_agents_returns_created(client):
    with patch("simulation._generate_bio", return_value="bio"):
        client.post("/api/simulation/agents", json=payload())
        client.post("/api/simulation/agents", json=payload())
    resp = client.get("/api/simulation/agents")
    assert resp.status_code == 200
...
def test_list_agents_newest_first(client):
    with patch("simulation._generate_bio", return_value="bio"):
        client.post("/api/simulation/agents", json=payload(name="First"))
        client.post("/api/simulation/agents", json=payload(name="Second"))
    resp = client.get("/api/simulation/agents")
    names = [a["name"] for a in resp.get_json()]
    assert names[0] == "Second"


# ── GET /api/simulation/agents/mine ───────────────────────────────────────────────

def test_mine_returns_agent(client):
    t = str(uuid.uuid4())
    with patch("simulation._generate_bio", return_value="bio"):
        client.post("/api/simulation/agents", json=payload(creator_token=t))
    resp = client.get(f"/api/simulation/agents/mine?creator_token={t}")
    assert resp.status_code == 200
    assert resp.get_json()["creator_token"] == t


def test_mine_returns_null_when_not_found(client):
    resp = client.get(f"/api/simulation/agents/mine?creator_token={uuid.uuid4()}")
    assert resp.status_code == 200
    assert resp.get_json() is None


def test_mine_missing_token_returns_400(client):
    resp = client.get("/api/simulation/agents/mine")
    assert resp.status_code == 400
